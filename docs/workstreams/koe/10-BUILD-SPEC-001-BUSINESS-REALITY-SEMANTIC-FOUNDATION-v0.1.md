# Build Spec 001 — Business Reality Semantic Foundation

**Version:** v0.1  
**Status:** KOE baseline for implementation  
...
Create the stable semantic foundation that allows GenSigma to represent, query, correct, and act on Business Reality across commercial, delivery, workforce, finance, compliance, and technology domains.

This is an enterprise foundation. Individual GenSigma episodes are validation fixtures only.
...
The current 38-object candidate architecture is defined in `09-BUSINESS-REALITY-V1-OBJECT-CATALOG-v0.1.md`.
...
The 38-object list is a candidate enterprise baseline, not a quota and not frozen schema.
...
### 4.5 Workforce / capability

```text
Person -> holds -> Position
Person -> has -> Skill | Credential | Certification
Organization | OrganizationalUnit | Person -> possesses -> Capability
Assignment -> requires -> Skill | Capability
Capability -> depends_on -> Person | System | Technology | Partner relationship | Credential
```
...
This catalog defines semantic families; final link names/cardinality are versioned contracts.
...
- No material canonical assertion without retrievable provenance.
...
7. Which people, skills, credentials, systems, technologies, and partner resources support a capability?
...
Build Spec 001 is not accepted against one episode.
...
Build 1 should implement the semantic kernel and contracts, not a domain-specific application.

The first codebase should therefore contain:
...
Domain-specific behavior may layer on top incrementally.

---

## 11. Build-readiness condition

The semantic kernel is ready for Codex when its contracts and acceptance tests are explicit enough that implementation cannot legally choose a different meaning for identity, role, evidence, time, correction, authority, or Event/Decision/Action/Outcome.
